...
from common.exploration import exploration_from_config
import torch
...
class PPO(BaseAgent):
    def __init__(self,
        ...
        self.ICE = exploration_from_config(exploration_config)
    ...
    def optimize(self):
        ...
        for _ in range(self.epoch):
            ...
            for sample in generator:
                ...
                loss = pi_loss.mean() - self.entropy_coef * entropy_loss + self.value_coef * value_loss 
                loss.backward()
                ...
                grad_accumulation_cnt += 1
                pi_loss_list.append(pi_loss.mean().item())
    ...
    def train(self, num_timesteps):
        ...
        while self.t < num_timesteps:
            ...
            for _ in range(self.n_steps):
                ...
                next_obs, rew_extrinsic, done, info = self.env.step(act)
                rew_intrinsic = self.ICE(next_obs, done)
                ice_info = self.ICE.get_episode_information()
                rew = rew_extrinsic + rew_intrinsic
                self.storage.store(obs, hidden_state, act, rew, rew_intrinsic, rew_extrinsic, done, info, ice_info, log_prob_act, value)
                obs = next_obs
            ...
            rew_batch, rew_intrinsic_batch, rew_extrinsic_batch, done_batch, ice_info_batch = self.storage.fetch_log_data()
            self.logger.feed(rew_batch, rew_intrinsic_batch, rew_extrinsic_batch, done_batch, ice_info_batch)
            self.logger.dump()
