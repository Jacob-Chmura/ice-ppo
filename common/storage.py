...
class Storage():
    ...
    def reset(self):
        ...
        self.rew_intrinsic_batch = torch.zeros(self.num_steps, self.num_envs)
        self.rew_extrinsic_batch = torch.zeros(self.num_steps, self.num_envs)
        ...
        self.ice_info_batch = deque(maxlen=self.num_steps)
        self.step = 0

    def store(self, obs, hidden_state, act, rew, rew_intrinsic, rew_extrinsic, done, info, ice_info, log_prob_act, value):
        ...
        self.rew_intrinsic_batch[self.step] = torch.from_numpy(rew_intrinsic.copy())
        self.rew_extrinsic_batch[self.step] = torch.from_numpy(rew_extrinsic.copy())
        ...
        self.ice_info_batch.append(ice_info)
    ...
    def compute_estimates(self, gamma=0.99, lmbda=0.95, use_gae=True, normalize_adv=True):
        ...
        #self.return_batch = rew_batch + gamma*self.value_batch[1:]
        if normalize_adv:
    ...
    def fetch_log_data(self):
        ...
        for step in range(self.num_steps):
            ice_info_batch.append(self.ice_info_batch[step]["Trajectory Information (bits)"])
        ice_info_batch = np.array(ice_info_batch)
        rew_intrinsic_batch = self.rew_intrinsic_batch.numpy()
        rew_extrinsic_batch = self.rew_extrinsic_batch.numpy()
        ...
        return rew_batch, rew_intrinsic_batch, rew_extrinsic_batch, done_batch, ice_info_batch